...
pub mod census2010 {
	pub mod pl94_171 {
		...
		macro_rules! field {
			($type:ident, $data_field_name:ident, $($name:ident @ { $loc:expr } - $getter_name:ident),+) => {
				$(
					#[allow(dead_code)]
			...
			($type:ident, $data_field_name:ident, $($name:ident @ { $loc:expr } - $vis:vis $getter_name:ident),+) => {
				$(
					#[allow(dead_code)]
					const $name: core::ops::Range<usize> = $loc;
